...
import (
	"fmt"
	"github.com/blocktree/lisk-adapter/api"
...
func NewLSKBlockScanner(wm *WalletManager) *LSKBlockScanner {
	bs := LSKBlockScanner{
...
func (bs *LSKBlockScanner) GetCurrentBlock() (*api.BlockResponse, error) {
...
func (bs *LSKBlockScanner) GetGlobalMaxBlockHeight() uint64 {
...
func (bs *LSKBlockScanner) GetBlockByHeight(height uint64) (*Block, error) {
	br := &api.BlockRequest{
		Height: int64(height),
	}
...
func (bs *LSKBlockScanner) GetScannedBlockHeader() (*openwallet.BlockHeader, error) {
...
func (bs *LSKBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
...
func (bs *LSKBlockScanner) GetScannedBlockHeight() uint64 {
	localHeight, _ := bs.wm.GetLocalNewBlock()
	return localHeight
}

//GetTransactionsByBlockHash
func (bs *LSKBlockScanner) GetTransactionsByBlock(block *Block) ([]*api.Transaction, error) {
...
//GetBlockHeight 获取区块链高度
func (bs *LSKBlockScanner) GetBlockHeight() (uint64, error) {
	//暂时只有一个接口获取钱包高度
	return bs.GetGlobalMaxBlockHeight(), nil
}

//newBlockNotify 获得新区块后，通知给观测者
func (bs *LSKBlockScanner) newBlockNotify(block *Block, isFork bool) {
...
func (bs *LSKBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *LSKBlockScanner) ExtractTransaction(block *Block, scanTargetFunc openwallet.BlockScanTargetFunc) (ExtractResult, error) {
...
func (bs *LSKBlockScanner) changeTrans(trans *api.Transaction, scanTargetFunc openwallet.BlockScanTargetFunc) (ExtractTxResult, error) {
	v := trans
	...
	switch txType {
	case successTxType:
		amountDec, err := decimal.NewFromString(v.Amount)
		if err != nil {
			log.Error("amount get error,err = ", err, "tx:", v.ID)
			return resultTx, err
		}
		feeDec, err := decimal.NewFromString(v.Fee)
		if err != nil {
			log.Error("amount get error,err = ", err, "tx:", v.ID)
			return resultTx, err
		}
...
func (bs *LSKBlockScanner) newExtractDataNotify(height uint64, extractTxResult []*ExtractTxResult) error {
...
func (bs *LSKBlockScanner) BatchExtractTransaction(block *Block) error {
...
func (bs *LSKBlockScanner) RescanFailedRecord() {
...
//LSKBlockScanner 扫描任务
func (bs *LSKBlockScanner) ScanBlockTask() {
